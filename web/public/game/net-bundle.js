...
${suffix}`;
  ...
  var RoomClient = class {
    ...
    async joinRoom(code, nickname) {
      ...
      const { data, error } = await this.supabase.from("rooms").select("code,status").eq("code", normalized).maybeSingle();
    ...
    members() {
      ...
      const members = Object.entries(state).map(
        ([clientId, metas]) => ({
          clientId,
    ...
    async markStarted() {
      ...
      await this.supabase.from("rooms").update({ status: "playing" }).eq("code", this.code);
    }
    async leave() {
      if (this.channel) {
      ...
      if (this.isHost && this.code) {
        await this.supabase.from("rooms").update({ status: "finished" }).eq("code", this.code);
      }
    }
  };
